...
每一列都帶車子自己的時間戳。50 台 AMR 的時鐘各差幾秒,事故分析就對不起來 ——
而且**完全不會報錯**,資料看起來一切正常,只是被歸到錯的時間點。

伺服器每一批量一次偏差(用信封的 `ts` 對伺服器收到的時間),然後:

| 欄位 | 是什麼 | 什麼時候用 |
|---|---|---|
| `ts` | 校正並夾限後的時間 | 分割、索引、單一車輛的走勢 |
| `recv_ts` | 伺服器實際收到的時間 | **跨車比對事故時間軸用這個** |
| `skew_ms` | 車子時鐘 − 伺服器時鐘 | 診斷;原始時間 = `ts + skew_ms` |

用信封而不是每列的 `ts` 來量,是因為斷線兩小時後補傳的資料**本來就**是兩小時
前的 —— 那是正確的回填,不是時鐘歪掉。信封的 `ts` 在送出當下才蓋,所以它把
「時鐘差多少」和「積壓多久」分開了。

夾限那一層不是為了準確,是為了保護分割鍵:`ts` 決定列落在哪個日分割,而
`topic_samples_default` **不會被保留期刪掉**(刪它會讓 insert 失敗)。一台時鐘
寫著 2031 年的車,它的資料會永久留存、無聲繞過整個保留期設計。

偏差超過 2 秒的車會列在管理頁的「系統」分頁,並寫一筆 `clock` 事件(每台車
只寫一次,不會每批都吵)。

**校正是補救,不是解法。** 在車上跑 `itri-agent doctor`,它會檢查
timedatectl / chrony 並告訴你怎麼修。
